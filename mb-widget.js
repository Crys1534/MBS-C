...
<div id="caja-comentarios">
  ...
  db.ref('comentarios/'+idDePagina).on('value',s=>{
      ...
      if(soloLikes){
          nC.forEach(c=>{
              let lBtn = document.querySelector(`.btn-corazon[data-id="${c.id}"]:not([data-pid]) span:not(.corazon-icono)`);
              if(lBtn && c.likes !== undefined) lBtn.textContent = c.likes;
              if(c.respuestas){
                  Object.keys(c.respuestas).forEach(k=>{
                      let rBtn = document.querySelector(`.btn-corazon[data-id="${k}"][data-pid="${c.id}"] span:not(.corazon-icono)`);
  ...
  DOM.lista.addEventListener('click',e=>{
    ...
    if(btn.classList.contains('btn-corazon')){
      ...
      const spanV = btn.querySelector('span:not(.corazon-icono)');
      ...
      if(st==='partial'||st==='none'){allR.forEach(el=>el.style.display='flex');btn.dataset.state='all';btn.innerHTML=`&#x21B3; Ocultar respuestas`;}else{allR.forEach(el=>el.style.display='none');btn.dataset.state='none';btn.innerHTML=`&#x21B3; Ver ${tot} respuesta${tot>1?'s':''}`;}
    }
  });
