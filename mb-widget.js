...
<div id="caja-comentarios">
  ...
  function publicarMsj(t,pId=null,rInput=null){
    ...
    db.ref(pId?`comentarios/${idDePagina}/${pId}/respuestas`:`comentarios/${idDePagina}`).push(data).then(()=>{
      ...
      if(!pId){DOM.txt.value="";DOM.txt.style.height='42px';appState.imgAdjMain=null;DOM.prevBox.style.display='none';}else{if(rInput){rInput.value="";rInput.style.height='42px';}appState.imgAdjResp[pId]=null;const box=document.getElementById('prev-resp-'+pId);if(box)box.style.display='none';}
    });
  }
  ...
  function render(){
    ...
    DOM.lista.innerHTML=arr.map(d=>{
      ...
      return html+`</div><div class="zona-formulario-respuesta" id="form-resp-${d.id}" style="display:none;"><div class="formulario-respuesta"><div class="avatar" style="width:30px;height:30px;flex-shrink:0;"><img src="${DOM.avaAct.src}"></div><div class="input-con-emoji-wrap"><textarea id="input-resp-${d.id}" class="input-respuesta" placeholder="${T.r}" rows="1"></textarea><button class="btn-emoji-trigger" data-target="${d.id}">&#x1F600;</button></div><button class="btn-attach" data-action="img" data-id="${d.id}" title="Adjuntar Foto">&#x1F5BC;&#xFE0F;</button><button class="btn-enviar-respuesta btn-send-whatsapp-small" data-id="${d.id}"><svg viewBox="0 0 24 24" width="16" height="16" fill="white"><path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z"/></svg></button></div><div class="preview-adjunto" id="prev-resp-${d.id}" style="margin-left:46px;"><img src=""><button class="btn-quitar-adjunto" data-action="quitar-adj" data-id="${d.id}">X</button></div></div></div></div>`;
    }).join('');
