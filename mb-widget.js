...
<div id="caja-comentarios">
  ...
  const genHash=(arr)=>arr.map(c=>c.id+(c.texto||'')+c.aprobado+(c.respuestas?Object.keys(c.respuestas).map(k=>k+(c.respuestas[k].texto||'')+c.respuestas[k].aprobado).join(''):'')).join('|');
  ...
  DOM.lista.addEventListener('click',e=>{
    ...
    if(btn.classList.contains('btn-corazon')){
      ...
      const isS = btn.dataset.shown === 'true';
      const ext = document.querySelectorAll(`.resp-extra-${id}`);
      if(isS){
          ext.forEach(el=>el.style.display='none');
          btn.dataset.shown='false';
          btn.innerHTML=`&#x21B3; Ver ${ext.length} respuesta${ext.length>1?'s':''} m\u00E1s`;
      }else{
          ext.forEach(el=>el.style.display='flex');
