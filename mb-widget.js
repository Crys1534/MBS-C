...
:root {
  ...
  --accent-blue: #58a6ff;
  --text-muted: #8b949e;
  --font-stack: 'Droid Sans', system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif;
}
.main-wrap p { margin:0 auto 8px !important; }
#caja-comentarios * { font-family:var(--font-stack)!important; box-sizing:border-box; }
#caja-comentarios { max-width:700px; margin: 10px auto 8px 10px !important; color:#fff; background:0 0; }
...
.formulario-comentarios { display:flex; flex-direction:column; gap:12px; padding:15px 15px 0; border-radius:6px; position: relative; }
.fila-usuario { display:flex; align-items:center; gap:6px; }
...
.emoji-tab:hover { background: var(--c-bg-hover); color: #fff; }
.emoji-tab.active { color: var(--accent-blue); border-bottom-color: var(--accent-blue); }
.emoji-grid-wrap { flex: 1; overflow-y: auto; padding: 10px; display: none; }
.emoji-grid-wrap.active { display: block; }
...
.caja-respuestas.con-hilo::before { content:''; position:absolute; top:-25px; bottom:15px; left:-26px; width:2px; background:var(--c-border); border-radius:2px; z-index:0; }
...
<div id="caja-comentarios">
  ...
  <div class="formulario-comentarios">
    ...
    <div class="input-con-emoji-wrap">
        <textarea id="textoComentario" required></textarea>
        <button class="btn-emoji-trigger" data-target="main">&#x1F600;</button>
    </div>
    ...
    <div class="controles-formulario">
      ...
      <button id="btnEnviar" class="btn-send-whatsapp"><svg viewBox="0 0 24 24" width="22" height="22" fill="white"><path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z"/></svg></button>
    </div>
  </div>
...
<div id="globalEmojiPicker" class="emoji-picker-container">
    <div class="emoji-tabs">
        ...
        <div class="emoji-tab" data-tab="tab-emotes">Emotes</div>
    </div>
    <div class="emoji-grid-wrap active" id="tab-emojis">
        <div class="emoji-grid" id="gridEmojisCore"></div>
    </div>
    <div class="emoji-grid-wrap" id="tab-emotes">
...
(async function() {
  ...
  auth.onAuthStateChanged(u=>{appState.usr=u;DOM.nom.style.display=u?'none':'block';document.getElementById('nombreUsuarioLogueado').style.display=u?'block':'none';document.getElementById('btnGoogle').style.display=u?'none':'flex';document.getElementById('infoUsuario').style.display=u?'flex':'none';if(u){document.getElementById('nombreUsuarioLogueado').textContent=u.displayName;document.getElementById('textoUsuario').textContent=T.l;DOM.avaAct.src=u.photoURL||"/uploads/1/2/7/3/127300461/default-pfp_596.png";}else{DOM.avaAct.src=appState.pfpL||"/uploads/1/2/7/3/127300461/default-pfp_596.png";}});
  ...
  DOM.inpFile.onchange=e=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=ev=>{const i=new Image();i.onload=()=>{const c=document.createElement('canvas');c.width=80;c.height=80;const s=Math.min(i.width,i.height),x=(i.width-s)/2,y=(i.height-s)/2;c.getContext('2d').drawImage(i,x,y,s,s,0,0,80,80);appState.pfpL=c.toDataURL('image/jpeg',0.95);DOM.avaAct.src=appState.pfpL;localStorage.setItem('savedAvatar',appState.pfpL);};i.src=ev.target.result;};r.readAsDataURL(f);};
  document.getElementById('btnImgMain').onclick=()=>{targetActivoAdjunto='main';inImg.click();};

  inImg.onchange=e=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=ev=>{const i=new Image();i.onload=()=>{const c=document.createElement('canvas');let h=222;let w=Math.round((i.width/i.height)*h);c.width=w;c.height=h;c.getContext('2d').drawImage(i,0,0,w,h);let mime=f.type==='image/png'?'image/png':'image/jpeg';let q=mime==='image/jpeg'?0.85:undefined;setPreview(targetActivoAdjunto,c.toDataURL(mime,q));};i.src=ev.target.result;};r.readAsDataURL(f);inImg.value="";};
  ...
  document.querySelectorAll('.emoji-tab').forEach(tab => {
      tab.onclick = () => {
          ...
          document.querySelectorAll('.emoji-grid-wrap').forEach(w => w.classList.remove('active'));
  ...
  picker.addEventListener('click', e => {
      ...
      if(item && currentInputTarget) {
          ...
          const start = currentInputTarget.selectionStart;
          const end = currentInputTarget.selectionEnd;
          const txt = currentInputTarget.value;
  ...
  db.ref('comentarios/'+idDePagina).on('value',s=>{appState.coms=[];s.forEach(c=>{const d=c.val();if(typeof d==='object'&&d!==null)appState.coms.push({id:c.key,...d});});render();});
  ...
  DOM.lista.addEventListener('click',e=>{
    ...
    if(btn.classList.contains('btn-corazon')){
      ...
      if(h){
          ...
          let newVal = parseInt(spanVal.textContent||0)+1;
          spanVal.textContent = newVal;
      }
    }
    ...
    // Otros botones
      ...
      const inp=document.getElementById('input-resp-'+id);
      publicarMsj(inp.value,id,inp);document.getElementById('form-resp-'+id).style.display='none';
  ...
  function calculaT(f){
    ...
    else if(L==='en'){const ft=(v,u)=>`${v} ${u}${v>1?'s':''} ago`;return s>=31536000?ft(Math.floor(s/31536000),"year"):s>=2592000?ft(Math.floor(s/2592000),"month"):s>=86400?ft(Math.floor(s/86400),"day"):s>=3600?ft(Math.floor(s/3600),"hour"):s>=60?ft(Math.floor(s/60),"minute"):T.j;}
  ...
  function render(){
    ...
    DOM.lista.innerHTML=arr.map(d=>{
      ...
      let html=`<div class="comentario-individual ${d.anclado?'es-anclado':''}"><div class="avatar"><img src="${d.foto||'/uploads/1/2/7/3/127300461/default-pfp_596.png'}"></div><div class="comentario-contenido"><div class="cabecera-comentario"><span class="comentario-nombre">${nom}${bI} ${d.anclado?`<span class="badge-anclado">&#x1F4CC; ${T.p}</span>`:''}</span><span class="comentario-fecha">${calculaT(d.fecha||ahora)}</span></div><p class="comentario-texto">${procT(d.texto||'')}</p>${d.imagenAdjunta?`<img src="${d.imagenAdjunta}" class="img-adjunta">`:''}<div class="acciones-comentario"><button class="btn-accion btn-corazon ${l?'likeado':''}" data-id="${d.id}"><span class="corazon-icono"></span><span>${d.likes||0}</span></button><button class="btn-accion btn-responder" data-id="${d.id}">${T.b}</button></div><div class="caja-respuestas ${d.respuestas&&Object.keys(d.respuestas).length>0?'con-hilo':''}">`;
      
      if(d.respuestas){
        html+=Object.keys(d.respuestas).map(k=>{
          const r=d.respuestas[k];if(typeof r!=='object'||!r||(r.shadowbanned&&r.nombre!==miNom)||r.aprobado===false)return '';
          const rn=r.nombre||'Usuario',rnL=rn.replace(/[\.\$\#\[\]\/]/g,'_');const rl=localStorage.getItem(`liked_${d.id}_${k}`);
          return `<div class="respuesta-individual"><div class="avatar"><img src="${r.foto||'/uploads/1/2/7/3/127300461/default-pfp_596.png'}"></div><div><div class="cabecera-comentario"><span class="comentario-nombre">${rn}${appState.insig[rnL]?`<img src="/uploads/1/2/7/3/127300461/featured-user-icon_orig.png" class="badge-img">`:''}</span><span class="comentario-fecha">${calculaT(r.fecha||ahora)}</span></div><p class="comentario-texto">${procT(r.texto||'')}</p>${r.imagenAdjunta?`<img src="${r.imagenAdjunta}" class="img-adjunta">`:''}<div class="acciones-comentario" style="margin-top: 5px; margin-bottom: 0px;"><button class="btn-accion btn-corazon ${rl?'likeado':''}" data-id="${k}" data-pid="${d.id}"><span class="corazon-icono"></span><span>${r.likes||0}</span></button></div></div></div>`;
        }).join('');
      }
      
      return html+`</div>
      <div class="zona-formulario-respuesta" id="form-resp-${d.id}" style="display:none;">
          <div class="formulario-respuesta">
              <div class="avatar" style="width:30px;height:30px;flex-shrink:0;"><img src="${DOM.avaAct.src}"></div>
              <div class="input-con-emoji-wrap">
                  <textarea id="input-resp-${d.id}" class="input-respuesta" placeholder="${T.r}" rows="1"></textarea>
                  <button class="btn-emoji-trigger" data-target="${d.id}">&#x1F600;</button>
              </div>
              <button class="btn-attach" data-action="img" data-id="${d.id}" title="Adjuntar Foto">&#x1F5BC;&#xFE0F;</button>
              <button class="btn-enviar-respuesta btn-send-whatsapp-small" data-id="${d.id}"><svg viewBox="0 0 24 24" width="16" height="16" fill="white"><path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z"/></svg></button>
          </div>
          <div class="preview-adjunto" id="prev-resp-${d.id}" style="margin-left:46px;"><img src=""><button class="btn-quitar-adjunto" data-action="quitar-adj" data-id="${d.id}">X</button></div>
      </div>
      </div></div>`;
    }).join('');
