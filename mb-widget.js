...
<div id="caja-comentarios">
  ...
  DOM.lista.addEventListener('keydown',e=>{if(e.target.classList.contains('input-respuesta')&&e.key==='Enter'&&!e.shiftKey){e.preventDefault();e.target.parentElement.querySelector('.btn-enviar-respuesta').click();}});
  ...
  DOM.lista.addEventListener('click',e=>{
    ...
    if(btn.classList.contains('btn-corazon')){
      ...
      if(h){r.transaction(c=>(c||0)>0?c-1:0);localStorage.removeItem(sKey);btn.classList.remove('likeado');}else{r.transaction(c=>(c||0)+1);localStorage.setItem(sKey,'true');btn.classList.add('likeado');}
