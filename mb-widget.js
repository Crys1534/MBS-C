(async function() {
  ...
  // Creador del contenedor de menciones superpuesto
  ...
  widgetBox.innerHTML = `
<style>
:root{--c-bg-hover:rgba(255,255,255,0.1);--c-border:rgba(255,255,255,0.15);--accent-blue:#58a6ff;--text-muted:#8b949e;--font-stack:'Droid Sans',system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif;}
.main-wrap p{margin:0 auto 8px !important;}
#caja-comentarios *{font-family:var(--font-stack)!important;box-sizing:border-box;}
#caja-comentarios{max-width:700px;margin:10px auto 8px 10px !important;color:#fff;background:0 0;position:relative;}
.cabecera-principal{display:flex;justify-content:space-between;align-items:center;border-bottom:2px solid rgba(255,255,255,.3);padding-bottom:8px;margin-bottom:20px;flex-wrap:wrap;gap:10px;}
.titulo-comentarios{margin:0;color:#fff;}
#btnGoogle{background:#fff;color:#333;border:none;padding:6px 12px;border-radius:4px;cursor:pointer;font-weight:700;font-size:13px;display:flex;align-items:center;gap:5px;}
#btnGoogle::before{content:'G';color:#ea4335;font-weight:900;}
#btnSalir{background:var(--c-bg-hover);color:#fff;border:1px solid rgba(255,255,255,.3);padding:4px 8px;border-radius:4px;cursor:pointer;font-size:12px;margin-left:10px;}
.formulario-comentarios{display:flex;flex-direction:column;gap:6px;padding:15px 15px 0;border-radius:6px;}
.fila-usuario{display:flex;align-items:flex-start;gap:10px;}
#nombreUsuario{padding:10px;border:0 solid rgba(255,255,255,.3);border-radius:4px;font-size:14px;background:rgba(0,0,0,.2);color:#fff;width:100%;max-width:225px;outline:none;}
#nombreUsuarioLogueado{font-weight:700;font-size:14px;}
.input-contenedor{position:relative;width:100%;flex-grow:1;background:rgba(0,0,0,.2);border-radius:4px;min-height:42px;}
.input-backdrop{position:absolute;top:0;left:0;right:0;bottom:0;pointer-events:none;overflow:hidden;z-index:1;color:#fff;}
.textarea-transparente{position:relative;z-index:2;background:transparent!important;color:transparent!important;caret-color:#fff;width:100%;outline:none;resize:none;overflow:hidden;min-height:42px;display:block;}
.input-backdrop, .textarea-transparente{margin:0;border:none;padding:10px;font-size:14px;font-family:var(--font-stack)!important;line-height:1.5;white-space:pre-wrap;word-break:break-word;box-sizing:border-box;letter-spacing:normal;font-weight:normal;}
.textarea-transparente::placeholder{color:transparent;}
.backdrop-placeholder{color:var(--text-muted);}
...
.btn-send-whatsapp-small{width:34px;height:34px;}
.btn-send-whatsapp:hover, .btn-send-whatsapp-small:hover{background:#005bb5;}
...
.cabecera-comentario{display:flex;align-items:baseline;gap:8px;flex-wrap:wrap;}
...
#caja-comentarios div p.comentario-texto{margin:0 0 10px 0 !important;font-size:14px !important;line-height:1.5 !important;white-space:pre-wrap !important;word-break:break-word !important;color:#fff;}
.comentario-texto a{color:var(--accent-blue);font-weight:bold;text-decoration:none;}
.comentario-texto a:hover{filter:brightness(1.2);text-decoration:none;}
.mention-text{color:var(--accent-blue);cursor:pointer;background:rgba(88,166,255,0.1);border-radius:2px;}
.mention-text:hover{filter:brightness(1.2);}
.img-adjunta{max-height:256px;width:auto;max-width:100%;border-radius:6px;margin-bottom:10px;display:block;image-rendering:auto !important;}
...
.btn-responder{font-weight:bold;}
.caja-respuestas{margin-top:10px;padding-left:0;position:relative;}
.caja-respuestas.con-hilo::before{content:'';position:absolute;top:-56px;bottom:98px;left:-26px;width:2px;background:var(--c-border);border-radius:2px;z-index:0;}
.respuesta-individual{display:flex;gap:6px;margin-top:15px;position:relative;margin-left:10px;}
.respuesta-individual::before{content:'';position:absolute;top:15px;left:-36px;width:36px;height:2px;background:var(--c-border);border-bottom-left-radius:4px;z-index:0;}
...
.btn-corazon.likeado .corazon-icono{background-image:url('https://mineblocksstuffs.weebly.com/uploads/1/2/7/3/127300461/like-comment2_orig.png')!important;}
.btn-corazon:active .corazon-icono{transform:scale(.7) translateY(2px);}
.badge-img{width:12px;height:12px;margin-left:5px;}
...
.skeleton-title{width:30%;height:14px;margin-bottom:15px;}
.skeleton-line.short{width:60%;}
.btn-cargar-mas{background:rgba(0,0,0,0.2);color:var(--accent-blue);border:1px solid rgba(255,255,255,0.2);width:100%;padding:12px;border-radius:6px;cursor:pointer;font-weight:bold;margin-top:15px;transition:all 0.2s;font-size:14px;}
.btn-cargar-mas:hover{background:rgba(255,255,255,0.1);color:#fff;}
...
<div id="caja-comentarios">
  <div class="cabecera-principal">
    ...
    <div id="auth-container">
      ...
      <div id="infoUsuario" style="display:none;"><span id="textoUsuario"></span><button id="btnSalir"></button></div>
    </div>
  </div>
  <div class="formulario-comentarios">
    <div class="fila-usuario">
      <div class="avatar avatar-upload-btn" id="avatarUsuarioActual" title="Cambiar foto">
        <img src="/uploads/1/2/7/3/127300461/default-pfp_596.png">
    ...
    <div class="preview-adjunto" id="cajaPreviewAdjunto">
      <img id="imgPreviewAdjunto" src="">
      <button class="btn-quitar-adjunto" id="btnQuitarAdjunto">X</button>
    </div>
    <div class="controles-formulario">
      <button class="btn-attach" id="btnImgMain" title="Adjuntar Imagen">&#x1F5BC;&#xFE0F;</button>
      <button id="btnEnviar" class="btn-send-whatsapp">
  ...
  let idDePagina = rutaActual.replace(/^\/|\.html$/g, '');
  if (idDePagina === "" || idDePagina === "/") idDePagina = "inicio";
  idDePagina = idDePagina.replace(/[\.\$\#\[\]\/]/g, "_");
  ...
  document.querySelectorAll("meta").forEach(x => {
    if ((x.getAttribute("property") || "").replace(/\s/g, "") === "og:image") imgCover = x.getAttribute("content");
  });
  if (imgCover) db.ref("imagenes_paginas/" + idDePagina).set(imgCover);

  const DOM = {
    txtTit: document.getElementById('txtTitulo'),
    lista: document.getElementById('listaComentarios'),
    nom: document.getElementById('nombreUsuario'),
    txt: document.getElementById('textoComentario'),
    btnEnv: document.getElementById('btnEnviar'),
    prevBox: document.getElementById('cajaPreviewAdjunto'),
    prevImg: document.getElementById('imgPreviewAdjunto'),
    avaAct: document.querySelector('#avatarUsuarioActual img'),
    inpFile: document.getElementById('inputAvatarFile')
  };

  let appState = { usr: null, pfpL: null, imgAdjMain: null, imgAdjResp: {}, limite: 10, coms: [], insig: {}, shadow: {} };
  let targetActivoAdjunto = null;
  let uniqueUsers = [];
  ...
  inImg.type = 'file';
  inImg.accept = 'image/png, image/jpeg, image/webp';

  const localeDict = {
    en: {t:"comments",t1:"comment",g:"Sign in with Google",o:"Sign out",l:"Signed in with Google",u:"Username",c:"Write a comment...",r:"Write a reply...",b:"Reply",m:"Load more comments...",a:"ago",j:"Just now",p:"Pinned",s:"Comment sent!",y:"Year",mo:"Month",d:"Day",h:"Hour",mi:"Minute"},
    pt: {t:"coment\u00E1rios",t1:"coment\u00E1rio",g:"Entrar com o Google",o:"Sair",l:"Conectado com o Google",u:"Nome de usu\u00E1rio",c:"Escreva um coment\u00E1rio...",r:"Escreva uma resposta...",b:"Responder",m:"Carregar mais coment\u00E1rios...",a:"atr\u00E1s",j:"Agora mesmo",p:"Fixado",s:"Coment\u00E1rio enviado!",y:"Ano",mo:"M\u00EAs",d:"Dia",h:"Hora",mi:"Minuto"},
    es: {t:"comentarios",t1:"comentario",g:"Iniciar sesi\u00F3n con Google",o:"Cerrar sesi\u00F3n",l:"Conectado con Google",u:"Usuario",c:"Escribe un comentario...",r:"Escribe una respuesta...",b:"Responder",m:"Cargar m\u00E1s comentarios...",a:"hace",j:"Justo ahora",p:"Fijado",s:"Comentario enviado!",y:"A\u00F1o",mo:"Mes",d:"D\u00EDa",h:"Hora",mi:"Minuto"}
  };

  const nL = navigator.language.substring(0, 2);
  const L = localeDict[nL] ? nL : 'es';
  const T = localeDict[L];

  document.getElementById('btnGoogle').textContent = T.g;
  document.getElementById('btnSalir').textContent = T.o;
  DOM.nom.placeholder = T.u;
  document.getElementById('backdrop-main').innerHTML = `<span class="backdrop-placeholder">${T.c}</span>`;
  DOM.nom.value = localStorage.getItem('savedName') || '';
  appState.pfpL = localStorage.getItem('savedAvatar');
  DOM.avaAct.src = appState.pfpL || "/uploads/1/2/7/3/127300461/default-pfp_596.png";
  DOM.nom.oninput = e => localStorage.setItem('savedName', e.target.value);

  auth.onAuthStateChanged(u => {
    appState.usr = u;
    DOM.nom.style.display = u ? 'none' : 'block';
    document.getElementById('nombreUsuarioLogueado').style.display = 'none';
    document.getElementById('btnGoogle').style.display = u ? 'none' : 'flex';
    document.getElementById('infoUsuario').style.display = u ? 'flex' : 'none';
    if (u) {
      document.getElementById('textoUsuario').textContent = T.l;
      DOM.avaAct.src = u.photoURL || "/uploads/1/2/7/3/127300461/default-pfp_596.png";
    } else {
      DOM.avaAct.src = appState.pfpL || "/uploads/1/2/7/3/127300461/default-pfp_596.png";
    }
  });

  document.getElementById('btnGoogle').onclick = () => auth.signInWithPopup(new firebase.auth.GoogleAuthProvider()).catch(e => alert("Error: " + e.message));
  document.getElementById('btnSalir').onclick = () => auth.signOut();
  document.getElementById('avatarUsuarioActual').onclick = () => !appState.usr && DOM.inpFile.click();

  DOM.inpFile.onchange = e => {
    ...
    r.onload = ev => {
      ...
      i.onload = () => {
        ...
        let s = Math.min(i.width, i.height), x = (i.width - s) / 2, y = (i.height - s) / 2;
        c.width = 80; c.height = 80;
        c.getContext('2d').drawImage(i, x, y, s, s, 0, 0, 80, 80);
        appState.pfpL = c.toDataURL('image/jpeg', 0.95);
        DOM.avaAct.src = appState.pfpL;
        localStorage.setItem('savedAvatar', appState.pfpL);
      };
  ...
  inImg.onchange = e => {
    ...
    r.onload = ev => {
      ...
      i.onload = () => {
        const c = document.createElement('canvas');
        let h = i.height > 256 ? 256 : i.height;
        let w = Math.round((i.width / i.height) * h);
        c.width = w; c.height = h;
        c.getContext('2d').drawImage(i, 0, 0, w, h);
        let mime = f.type === 'image/png' ? 'image/png' : 'image/jpeg';
        let q = mime === 'image/jpeg' ? 0.85 : undefined;
        setPreview(targetActivoAdjunto, c.toDataURL(mime, q));
      };
      i.src = ev.target.result;
    };
    r.readAsDataURL(f);
  ...
  function setPreview(target, src) {
    if (target === 'main') {
      appState.imgAdjMain = src;
      DOM.prevImg.src = src;
      DOM.prevBox.style.display = 'block';
    } else {
      appState.imgAdjResp[target] = src;
      const box = document.getElementById('prev-resp-' + target);
      if (box) {
  ...
  function updateBackdrop(ta) {
    const tg = ta.dataset.target;
    const bd = document.getElementById('backdrop-' + tg);
    if (!bd) return;
    let v = ta.value;
    if (!v) { bd.innerHTML = `<span class="backdrop-placeholder">${tg === 'main' ? T.c : T.r}</span>`; return; }
    let safe = v.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
    uniqueUsers.forEach(u => {
      const eN = u.nombre.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
      const reg = new RegExp(`(^|\\s)@${eN}(?=\\s|$)`, 'g');
      safe = safe.replace(reg, `$1<span class="mention-text">@${u.nombre}</span>`);
    });
    if (safe.endsWith('\n')) safe += '<br>';
    bd.innerHTML = safe;
  }
  ...
  function publicarMsj(t, pId = null, rInput = null) {
    const cd = localStorage.getItem('floodT');
    if (cd && (Date.now() - cd) < 30000) { alert(`\u23F3 Spam.`); return; }
    ...
    db.ref(pId ? `comentarios/${idDePagina}/${pId}/respuestas` : `comentarios/${idDePagina}`).push(data).then(() => {
      ...
      const mE = document.getElementById('msgExito');
      mE.textContent = "\u2705 " + T.s;
      mE.style.display = 'block';
      setTimeout(() => mE.style.display = 'none', 5000);
      if (!pId) {
        DOM.txt.value = "";
        DOM.txt.style.height = '42px';
        updateBackdrop(DOM.txt);
        appState.imgAdjMain = null;
        DOM.prevBox.style.display = 'none';
      } else {
        if (rInput) {
          rInput.value = "";
          rInput.style.height = '42px';
          updateBackdrop(rInput);
        }
        appState.imgAdjResp[pId] = null;
        const box = document.getElementById('prev-resp-' + pId);
  ...
  function updateUniqueUsers() {
    const map = {};
    appState.coms.forEach(c => {
      if (c.nombre && c.foto) map[c.nombre] = c.foto;
      if (c.respuestas) {
        Object.values(c.respuestas).forEach(r => { if (r.nombre && r.foto) map[r.nombre] = r.foto; });
      }
    });
    uniqueUsers = Object.keys(map).map(k => ({ nombre: k, foto: map[k] }));
  }
  ...
  db.ref('comentarios/' + idDePagina).on('value', s => {
      let nC = [];
      s.forEach(c => { const d = c.val(); if (typeof d === 'object' && d !== null) nC.push({ id: c.key, ...d }); });
      ...
      if (soloLikes) {
          nC.forEach(c => {
              ...
              if (c.respuestas) {
                  Object.keys(c.respuestas).forEach(k => {
                      let rBtn = document.querySelector(`.btn-corazon[data-id="${k}"][data-pid="${c.id}"] .contador-likes`);
                      if (rBtn && c.respuestas[k].likes !== undefined) rBtn.textContent = c.respuestas[k].likes;
                  });
              }
          });
      } else { render(); }
  ...
  document.addEventListener('input', e => {
    if (e.target.tagName === 'TEXTAREA' && (e.target.id === 'textoComentario' || e.target.classList.contains('input-respuesta'))) {
        ...
        if (match && match[1].length < 30) {
            ...
            mentionStartIndex = cursor - searchStr.length - 1;
  ...
  document.addEventListener('click', e => {
    ...
    if (item && activeTextarea) {
        ...
        const after = val.substring(activeTextarea.selectionStart);
        const insertText = `@${name} `;
        activeTextarea.value = before + insertText + after;
        mBox.style.display = 'none';
        activeTextarea.focus();
        const newPos = mentionStartIndex + insertText.length;
        ...
        updateBackdrop(activeTextarea);
    } else if (!e.target.closest('#mentionsDropdown') && e.target.tagName !== 'TEXTAREA') {
        mBox.style.display = 'none';
    }
  });
  ...
  DOM.lista.addEventListener('click', e => {
    ...
    if (btn.classList.contains('btn-corazon')) {
      ...
      if (h) {
          ...
          spanV.textContent = Math.max(0, parseInt(spanV.textContent || 0) - 1);
          localStorage.removeItem(sKey);
          r.transaction(c => (c || 0) > 0 ? c - 1 : 0);
      } else {
          btn.classList.add('likeado');
          spanV.textContent = parseInt(spanV.textContent || 0) + 1;
          localStorage.setItem(sKey, 'true');
          r.transaction(c => (c || 0) + 1);
      }
    } else if (btn.classList.contains('btn-responder')) {
      const z = document.getElementById('form-resp-' + id);
      const ta = z.querySelector('.input-respuesta');
      const rU = btn.dataset.replyuser;
      if (z.style.display === 'none' || ta.dataset.lastu !== rU) {
        z.style.display = 'block';
        ta.dataset.lastu = rU;
        if (rU) {
            const m = `@${rU} `;
            if (!ta.value.startsWith(m)) {
                ta.value = m;
                updateBackdrop(ta);
            }
        }
      ...
      const inp = btn.parentElement.querySelector('.input-respuesta');
      publicarMsj(inp.value, id, inp);
      document.getElementById('form-resp-' + id).style.display = 'none';
    } else if (btn.dataset.action === 'img') {
      targetActivoAdjunto = id;
      inImg.click();
    } else if (btn.dataset.action === 'quitar-adj') {
      appState.imgAdjResp[id] = null;
      document.getElementById('prev-resp-' + id).style.display = 'none';
    } else if (btn.classList.contains('btn-cargar-mas')) {
      appState.limite += 10;
      render();
    } else if (btn.classList.contains('btn-toggle-resp')) {
      const st = btn.dataset.state;
      const tot = parseInt(btn.dataset.total);
      const allR = document.querySelectorAll(`.resp-item-${id}`);
      if (st === 'partial' || st === 'none') {
          allR.forEach(el => el.style.display = 'flex');
          btn.dataset.state = 'all';
          btn.innerHTML = `&#x21B3; Ocultar respuestas`;
      } else {
          allR.forEach(el => el.style.display = 'none');
          btn.dataset.state = 'none';
          btn.innerHTML = `&#x21B3; Ver ${tot} respuesta${tot > 1 ? 's' : ''}`;
      }
    }
  });
  ...
  function calculaT(f) {
    ...
    if (L === 'es') {
        const ft = (v, u) => `hace ${v} ${u}${v > 1 ? (u === 'mes' ? 'es' : 's') : ''}`;
        return s >= 31536000 ? ft(Math.floor(s / 31536000), "a\u00F1o") : s >= 2592000 ? ft(Math.floor(s / 2592000), "mes") : s >= 86400 ? ft(Math.floor(s / 86400), "d\u00EDa") : s >= 3600 ? ft(Math.floor(s / 3600), "hora") : s >= 60 ? ft(Math.floor(s / 60), "minuto") : T.j;
    } else if (L === 'en') {
        const ft = (v, u) => `${v} ${u}${v > 1 ? 's' : ''} ago`;
        return s >= 31536000 ? ft(Math.floor(s / 31536000), "year") : s >= 2592000 ? ft(Math.floor(s / 2592000), "month") : s >= 86400 ? ft(Math.floor(s / 86400), "day") : s >= 3600 ? ft(Math.floor(s / 3600), "hour") : s >= 60 ? ft(Math.floor(s / 60), "minuto") : T.j;
    } else {
        const ft = (v, u) => `${v} ${u}${v > 1 ? 's' : ''} atr\u00E1s`;
        return s >= 31536000 ? ft(Math.floor(s / 31536000), "ano") : s >= 2592000 ? ft(Math.floor(s / 2592000), "m\u00EAs") : s >= 86400 ? ft(Math.floor(s / 86400), "dia") : s >= 3600 ? ft(Math.floor(s / 3600), "hora") : s >= 60 ? ft(Math.floor(s / 60), "minuto") : T.j;
    }
  }

  function render() {
    let cTotal = 0;
    const miNom = appState.usr ? appState.usr.displayName : (DOM.nom.value.trim() || "Invitado");
    appState.coms.forEach(c => {
        if (c.aprobado !== false) {
            cTotal++;
            if (c.respuestas) { Object.values(c.respuestas).forEach(r => { if (r.aprobado !== false) cTotal++; }); }
        }
    });
    ...
    const arr = appState.coms.slice(0, appState.limite);
    ...
    DOM.lista.innerHTML = arr.map(d => {
      ...
      if (d.respuestas) {
        ...
        html += vK.map((k, idx) => {
          const r = d.respuestas[k];
          const rn = r.nombre || 'Usuario', rnL = rn.replace(/[\.\$\#\[\]\/]/g, '_');
          const rl = localStorage.getItem(`liked_${d.id}_${k}`);
