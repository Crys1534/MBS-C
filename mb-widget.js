...
<div id="caja-comentarios">
  ...
  inImg.onchange=e=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=ev=>{const i=new Image();i.onload=()=>{const c=document.createElement('canvas');let h=i.height>296?296:i.height;let w=Math.round((i.width/i.height)*h);c.width=w;c.height=h;c.getContext('2d').drawImage(i,0,0,w,h);let mime=f.type==='image/png'?'image/png':'image/jpeg';let q=mime==='image/jpeg'?0.85:undefined;setPreview(targetActivoAdjunto,c.toDataURL(mime,q));};i.src=ev.target.result;};r.readAsDataURL(f);inImg.value="";};
